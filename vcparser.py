...
def parse(parse_table, token_list):
    ...
    while len(stack) > 0:
        ...
        if stack[-1] == current_token:
            ...
            # print(temp[0])
            stack.pop()
            filtered_temp = [item for item in temp[0] if "program" not in item and ";" not in item and "stmt" not in item]
            contains_operators = any(op in temp[0] for op in current_token)
...
if __name__ == '__main__':
    ...
    parser.add_argument("lexer_data", nargs="?", default="dfa.json")
